...
<?php
        ...
        if(isset($_GET['place']))
                 {  
                    $query="DELETE FROM favorite where Customer='$customer'";
        ...
        if(isset($_GET['remove']))
                 {  $product=$_GET['remove'];
                    $query="DELETE FROM favorite where Customer='$customer' AND Product='$product'";
                    $result=mysqli_query($conn,$query);
                 ?>
                    <script type="text/javascript">
...
<head>
    ...
    <style>
        #favorite {margin-top:30px;margin-bottom:30px;}
        .panel {border:1px solid #D67B22;padding-left:0px;padding-right:0px;}
        .panel-heading {background:rgb(61, 158, 228) !important;color:white !important;}        
...
echo '<div class="container-fluid" id="cart">
      <div class="row">
          <div class="col-xs-12 text-center" id="heading">
                 <h2 style="color:rgb(61, 158, 228);text-transform:uppercase;">  YOUR FAVORITES </h2>
           </div>
        </div>';
	if(isset($_SESSION['username']))
	    {      
	            ...
	            {   
                        ...
                        $query="SELECT * from favorite where Customer='$customer' AND Product='$product'";
                        $result=mysqli_query($conn,$query);
                        $row = mysqli_fetch_assoc($result);
                        if(mysqli_num_rows($result)==0)
	                         { $query="INSERT INTO favorite values('$customer','$product','$quantity')"; 
                              $result=mysqli_query($conn,$query);
                            }
                        else
                           { $new=$_GET['quantity'];
                             $query="UPDATE `favotite` SET Quantity=$new WHERE Customer='$customer' AND Product='$product'";
	                           $result=mysqli_query($conn,$query);
                            }
                    }
              	$query="SELECT PID,Title,Author,Edition,Quantity,Price FROM favorite INNER JOIN products ON favorite.Product=products.PID 
                             ...
                             if($j%2==0)
                                 ...
                                 echo '                
                                      <div class="panel col-xs-12 col-sm-4 col-sm-offset-'.$offset.' col-md-4 col-md-offset-'.$offset.' col-lg-4 col-lg-offset-'.$offset.' text-center" style="color:#D67B22;font-weight:800;">
                                          <div class="panel-heading">Favorite '. $i .'
                                          </div>
                                          <div class="panel-body">
			                                                <img class="image-responsive block-center" src="'.$path.'" style="height :100px;"> <br>
                                                                        ...
                                                                        Code : '.$row['PID'].'     <br>        	 
                                                      									...
                                                      									Price : '.$row['Price'].' <br>
                                                      									Sub Total : '.$Stotal.' <br>
                                                                       <a href="favorite.php?remove='.$row['PID'].'" class="btn btn-sm" 
                                                                          style="background:#d9534f;color:white;font-weight:800;">
                                                                          Remove
                                                                        </a>
                                        </div>
                                    </div>';
                    ...
                    echo '<div class="container">
                     ...
                     echo '<div class="row">
                             <div class="col-xs-8 col-xs-offset-2  col-sm-4 col-sm-offset-2 col-md-4 col-md-offset-3 col-lg-4 col-lg-offset-3">
                                  <a href="userhome.php" class="btn btn-lg" style="background:#d9534f;color:white;font-weight:800;">Continue Shopping</a>
                             </div>
                             <div class="col-xs-6 col-xs-offset-3 col-sm-4 col-sm-offset-2 col-md-4 col-md-offset-1 col-lg-4 ">
                                  <a href="favorite.php?place=true" class="btn btn-lg" style="background:#d9534f;color:white;font-weight:800;margin-top:5px;">Place Order</a>
                             </div>
                           </div>
                           ';
                } 
                        ...
                        echo ' 
                         <div class="row">
                            <div class="col-xs-9 col-xs-offset-3 col-sm-4 col-sm-offset-5 col-md-4 col-md-offset-5">
                                 <span class="text-center" style="color:#d9534f;font-weight:bold;">&nbsp &nbsp &nbsp &nbspFavorite Is Empty</span>
                             </div>
                         </div>
                         <div class="row">
                             <div class="col-xs-9 col-xs-offset-3 col-sm-2 col-sm-offset-5 col-md-2 col-md-offset-5">
                                  <a href="userhome.php" class="btn btn-lg" style="background:#d9534f;color:white;font-weight:800;">Add Some Favorite</a>
                             </div>
                          </div>';
                     }               
	    }
